...
export async function registerUser(req, res) {
  try {
    ...
    if (tipo === "fisica") {
      ...
      // Verifica se já existe CPF (compara sem formatação)
      const allUsers = await UserModel.findAll();
      const cpfClean = cpf.replace(/\D/g, "");
      const existingCPF = allUsers.find(u => (u.cpf || "").replace(/\D/g, "") === cpfClean);
      if (existingCPF) {
    ...
    if (tipo === "juridica") {
      ...
      // Verifica se já existe CNPJ (compara sem formatação)
      const allUsers = await UserModel.findAll();
      const cnpjClean = cnpj.replace(/\D/g, "");
      const existingCNPJ = allUsers.find(u => (u.cnpj || "").replace(/\D/g, "") === cnpjClean);
      if (existingCNPJ) {
        return res.status(409).json({ erro: "Este CNPJ já está cadastrado!" });
      }
    }

    // Gera ID temporário para o formulário e retorna para frontend
...
export async function formularioCompleto(req, res) {
  try {
    ...
    if (tipoCadastro === "fisica") {
      ...
      // Verifica se CPF já está cadastrado (compara sem formatação)
      const existingCPF = allUsers.find(u => (u.cpf || "").replace(/\D/g, "") === cpfClean);
      if (existingCPF) {
        return res.status(409).json({ erro: "Este CPF já está cadastrado!" });
      }
      ...
      // Verifica se CNPJ já está cadastrado (compara sem formatação)
      const existingCNPJ = allUsers.find(u => (u.cnpj || "").replace(/\D/g, "") === cnpjClean);
      if (existingCNPJ) {
        return res.status(409).json({ erro: "Este CNPJ já está cadastrado!" });
      }

      // Inscrição estadual é obrigatória para pessoa jurídica
      if (!inscricaoEstadual || !inscricaoEstadual.trim()) {
        return res.status(400).json({ erro: "Inscrição estadual é obrigatória para pessoa jurídica" });
      }
      // Verifica se inscrição estadual já está cadastrada
      const inscricaoClean = inscricaoEstadual.replace(/\D/g, "");
      const existingInscricao = allUsers.find(u => 
        u.formulario && (u.formulario.inscricaoEstadual || "").replace(/\D/g, "") === inscricaoClean
      );
      if (existingInscricao) {
        return res.status(409).json({ erro: "Esta inscrição estadual já está cadastrada!" });
      }
    } else {
